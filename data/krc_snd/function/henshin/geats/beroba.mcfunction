...
advancement grant @s only krc_snd:henshin/geats/beroba_seq 1
advancement revoke @s from krc_snd:henshin/geats/standby_root
scoreboard players set @s krc.seq1 0
execute if score @s krc.form1n matches 0 run playsound kamenridercraft:laser_on player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~ 3
execute if score @s krc.form1n matches 1 run playsound kamenridercraft:laser_on_premium player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~ 3
title @a[scores={krc.configs.sound_subs=1},distance=..20] actionbar {"translate":"sound.kamenridercraft.geats.laser_on","color":"light_purple"}
