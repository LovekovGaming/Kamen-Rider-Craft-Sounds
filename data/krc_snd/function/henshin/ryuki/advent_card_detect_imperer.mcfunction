advancement revoke @s only krc_snd:henshin/ryuki/advent_card_detect_imperer
execute as @n[type=item,predicate=krc_snd:valid_item,distance=..2] if items entity @s contents kamenridercraft:gazelle_stab unless score @s krc.seq1 matches 0.. store success score @s krc.seq1 run data get entity @s Thrower
...
execute if entity @s[tag=advent] if items entity @n[type=item,predicate=krc_snd:valid_item,distance=..2] contents kamenridercraft:gazelle_stab run tag @s add spin_vent