advancement revoke @s only krc_snd:henshin/ryuki/advent_card_detect_time
execute if score @s krc.seq1 matches 0 if score @s krc.configs.ryuki_type matches 0 run playsound kamenridercraft:visor_close player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~ 3
...
execute if score @s krc.seq1 matches 0 run scoreboard players reset @s krc-advent.time
...
execute if score @s krc.seq1 matches 0 run tag @s add time_vent