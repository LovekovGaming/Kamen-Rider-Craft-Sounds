advancement revoke @s only krc_snd:henshin/ryuki/advent_card_detect_verde
execute as @n[type=item,predicate=krc_snd:valid_item,distance=..2] if items entity @s contents kamenridercraft:bio_winder unless score @s krc.seq1 matches 0.. store success score @s krc.seq1 run data get entity @s Thrower
...
execute if entity @s[tag=advent] if items entity @n[type=item,predicate=krc_snd:valid_item,distance=..2] contents kamenridercraft:bio_winder run tag @s add hold_vent