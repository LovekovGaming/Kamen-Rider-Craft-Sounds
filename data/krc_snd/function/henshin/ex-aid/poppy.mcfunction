...
execute if entity @s[tag=!no_gashat] run playsound kamenridercraft:gashat_poppy player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~ 3
execute if entity @s[tag=!no_gashat] run title @a[scores={krc.configs.sound_subs=1},distance=..20] actionbar [{"translate":"sound.kamenridercraft.ex-aid.gashat","color":"light_purple"}," ",{"translate":"sound.kamenridercraft.ex-aid.buggle_up","color":"aqua"}]
execute if entity @s[tag=no_gashat] run scoreboard players set @s krc.seq1 24
advancement grant @s only krc_snd:henshin/ex-aid/poppy_seq 1
advancement revoke @s from krc_snd:henshin/common/detransform_root
advancement grant @s only krc_snd:henshin/common/detransform_root
advancement grant @s only krc_snd:henshin/ex-aid/gashacon_bugvisor_ii_off 1_poppy