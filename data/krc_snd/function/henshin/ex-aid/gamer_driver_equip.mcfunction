...
execute unless predicate krc_core:sneaking if entity @s[advancements={krc_core:player_transformed=false}] run playsound kamenridercraft:gamer_driver_equip player @a[scores={krc.configs.equip_snd=1}] ~ ~1 ~ 3
execute if predicate krc_core:sneaking run playsound kamenridercraft:belt_equip player @a[scores={krc.configs.equip_snd=1}] ~ ~1 ~ 3
