...
execute if score @s krc.form2n matches 0..1 run playsound kamenridercraft:necrom_detransform player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
execute unless score @s krc.form2n matches 0..1 run advancement grant @s only krc_snd:henshin/ghost/gamma_superior_seq 1
execute unless score @s krc.form2n matches 0..1 run playsound kamenridercraft:proto_ulorder_loading player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
