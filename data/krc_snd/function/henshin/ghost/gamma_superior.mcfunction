...
execute if score @s krc.form2n matches 0..1 run function krc_snd:play_global {name:"kamenridercraft:necrom_detransform",scope:"henshin_snd"}
execute unless score @s krc.form2n matches 0..1 run advancement grant @s only krc_snd:henshin/ghost/gamma_superior_seq 1
execute unless score @s krc.form2n matches 0..1 run function krc_snd:play_global {name:"kamenridercraft:proto_ulorder_loading",scope:"henshin_snd"}
