...
execute at @e[type=kamenridercraft:bitter_gavv,tag=!sounded,tag=sound_active,scores={krc.seq1=39}] run playsound kamenridercraft:sparkingummy hostile @a[scores={krc.configs.mob_snd=1}] ~ ~1 ~

execute if entity @e[type=kamenridercraft:bitter_gavv,tag=!sounded,tag=sound_active] run schedule function krc_snd:mobs/reiwa/bitter_gavv_seq 1t replace
