...
execute at @e[type=kamenridercraft:dread,tag=sounded_2,tag=sound_active,scores={krc.seq1=48}] run playsound kamenridercraft:dread_sanshiki hostile @a[scores={krc.configs.mob_snd=1}] ~ ~1 ~

execute if entity @e[type=kamenridercraft:dread,tag=!sounded,tag=sound_active] run schedule function krc_snd:mobs/reiwa/dread_sanshiki_seq 1t replace
