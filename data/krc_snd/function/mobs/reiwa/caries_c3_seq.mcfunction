...
execute at @e[type=kamenridercraft:caries,tag=sounded_2,tag=sound_active,scores={krc.seq1=52}] run playsound kamenridercraft:caries_c3 hostile @a[scores={krc.configs.mob_snd=1}] ~ ~1 ~

execute if entity @e[type=kamenridercraft:caries,tag=!sounded,tag=sound_active] run schedule function krc_snd:mobs/reiwa/caries_c3_seq 1t replace
