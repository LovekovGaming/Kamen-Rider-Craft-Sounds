...
execute as @n[type=item,distance=..5,predicate=krc_snd:valid_item] if items entity @s contents kamenridercraft:vicarya_zetsumerisekey run tag @s add valid
...
execute if score @s krc.henshin-stage matches 1 if entity @n[type=item,distance=..5,tag=valid] run playsound kamenridercraft:vicarya_zetsumerisekey player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~ 3
