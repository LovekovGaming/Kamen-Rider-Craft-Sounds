...
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:shiita_mimic_key run playsound kamenridercraft:mimic_key_eject player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
