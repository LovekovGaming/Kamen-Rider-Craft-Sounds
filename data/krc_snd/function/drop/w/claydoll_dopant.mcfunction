...
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:claydoll_memory run playsound kamenridercraft:claydoll_dopant player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
