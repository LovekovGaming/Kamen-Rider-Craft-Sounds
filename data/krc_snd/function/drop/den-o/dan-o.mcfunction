...
execute if score @s krc.henshin-stage matches 1 if entity @n[type=item,distance=..5,tag=valid] if items entity @n[type=item,distance=..5,tag=valid] contents kamenridercraft:rider_pass run playsound kamenridercraft:den-o_belt_buckle_plat player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
