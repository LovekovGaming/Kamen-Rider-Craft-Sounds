...
execute unless score @s krc.form1n matches 1..3 as @n[type=item,distance=..5,predicate=krc_snd:valid_item] if items entity @s contents kamenridercraft:gatack_zecter run tag @s add valid
execute unless score @s krc.form1n matches 1..3 as @n[type=item,distance=..5,predicate=krc_snd:valid_item] if items entity @s contents kamenridercraft:gatack_zecter run tag @s add gatack_zecter
...
execute if score @s krc.henshin-stage matches 1 unless predicate krc_core:heisei/kabuto_armor if entity @n[type=item,distance=..5,tag=gatack_zecter] run function krc_snd:play_global {name:"kamenridercraft:gatack_zecter_appear",scope:"henshin_snd"}
execute if score @s krc.henshin-stage matches 1 if predicate krc_core:heisei/kabuto_armor if entity @n[type=item,distance=..5,tag=gatack_zecter] run function krc_snd:play_global {name:"kamenridercraft:gatack_cast_off_start",scope:"henshin_snd"}
...
execute if score @s krc.henshin-stage matches 3 if entity @n[type=item,distance=..5,tag=valid] run stopsound @a[scores={krc.configs.henshin_snd=1},distance=..20] player kamenridercraft:gatack_cast_off_start
execute if score @s krc.henshin-stage matches 3 if entity @n[type=item,distance=..5,tag=valid] run stopsound @a[scores={krc.configs.henshin_snd=1},distance=..20] player kamenridercraft:gatack_cast_off_standby
