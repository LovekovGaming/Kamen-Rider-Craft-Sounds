...
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:kickhopper_zecter run playsound kamenridercraft:zect_buckle_open player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
execute if score @s krc.henshin-stage matches 2 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:kickhopper_zecter run playsound kamenridercraft:hopper_zecter_appear player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
