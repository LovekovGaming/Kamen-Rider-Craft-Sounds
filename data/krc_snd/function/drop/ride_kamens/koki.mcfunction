...
execute if score @s krc.henshin-stage matches 1 unless score @s krc.configs.ride_kamens matches 1 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:chaos_ring_koki run playsound kamenridercraft:chaos_ring_on player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
