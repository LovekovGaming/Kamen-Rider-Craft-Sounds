...
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,tag=valid] contents #kamenridercraft:gear/form_items/gamma_superior run playsound kamenridercraft:eyecon_press player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,tag=valid] contents kamenridercraft:knife_gamma_eyecon run playsound kamenridercraft:knife_eyecon player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
