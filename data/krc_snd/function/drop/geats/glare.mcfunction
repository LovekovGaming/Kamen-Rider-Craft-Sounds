...
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:providence_card_glare run playsound kamenridercraft:glare_login player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~ 3
