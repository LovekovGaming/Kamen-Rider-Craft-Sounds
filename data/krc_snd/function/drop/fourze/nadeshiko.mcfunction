...
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:nadeshiko_switch run playsound kamenridercraft:fourze_switch1 player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
execute if score @s krc.henshin-stage matches 2 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents kamenridercraft:nadeshiko_switch run playsound kamenridercraft:fourze_switch4 player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
