...
execute if score @s krc.henshin-stage matches 1 if items entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] contents #kamenridercraft:gear/fuestles run playsound kamenridercraft:fangire_reveal player @a[scores={krc.configs.henshin_snd=1}] ~ ~1 ~
...
execute if score @s krc.henshin-stage matches 2.. if entity @n[type=item,distance=..5,predicate=krc_snd:valid_item] run scoreboard players reset @s krc.henshin-stage
